...
exports.signup = function (req, res, next) {
  ...
  if (!tools.validateId(loginname)) {
    return ep.emit('prop_err', 'Username is not valid');
  }
  if (!validator.isEmail(email)) {
    return ep.emit('prop_err', 'Email is not valid');
  }
  if (pass !== rePass) {
    return ep.emit('prop_err', 'Passwords you typed do not match');
  }
  // END 验证信息的正确性
  ...
  User.getUsersByQuery({'$or': [
    ...
    if (users.length > 0) {
      ep.emit('prop_err', 'The username or Email address is already in use.');
      return;
    }
...
exports.login = function (req, res, next) {
  ...
  ep.on('login_error', function (login_error) {
    res.status(403);
    res.render('sign/signin', { error: 'Incorrect username or password' });
  });
  ...
  getUser(loginname, function (err, user) {
    ...
    tools.bcompare(pass, passhash, ep.done(function (bool) {
      ...
      if (!user.active) {
        ...
        res.status(403);
        return res.render('sign/signin', { error: 'This account has not been activated yet, the activation link has been sent to ' + user.email + ' ，please check your Email.' });
      }
...
exports.activeAccount = function (req, res, next) {
  ...
  User.getUserByLoginName(name, function (err, user) {
    ...
    if (!user || utility.md5(user.email + passhash + config.session_secret) !== key) {
      return res.render('notify/notify', {error: 'Incorrect information，account cannot be activated.'});
    }
    if (user.active) {
    ...
    user.save(function (err) {
      ...
      res.render('notify/notify', {success: 'The account is already active, please log in.'});
    });
  });
};
...
exports.updateSearchPass = function (req, res, next) {
  var email = validator.trim(req.body.email).toLowerCase();
  if (!validator.isEmail(email)) {
    return res.render('sign/search_pass', {error: 'Email is not valid', email: email});
  }
  ...
  User.getUserByMail(email, function (err, user) {
    if (!user) {
      res.render('sign/search_pass', {error: 'Do not find this Email address', email: email});
      return;
    }
    ...
    user.save(function (err) {
      ...
      res.render('notify/notify', {success: 'We have sent an email to your registered email address. Please click the link to reset your password within 30 minutes.'});
    });
  });
};
...
exports.resetPass = function (req, res, next) {
  ...
  User.getUserByNameAndKey(name, key, function (err, user) {
    if (!user) {
      res.status(403);
      return res.render('notify/notify', {error: 'Incorrect information, password cannot be reset.'});
    }
    var now = new Date().getTime();
    var oneDay = 1000 * 60 * 60 * 24;
    if (!user.retrieve_time || now - user.retrieve_time > oneDay) {
      res.status(403);
      return res.render('notify/notify', {error: 'This link has expired, please reapply.'});
    }
...
exports.updatePass = function (req, res, next) {
  ...
  if (psw !== repsw) {
    return res.render('sign/reset', {name: name, key: key, error: 'Passwords you typed do not match'});
  }
  User.getUserByNameAndKey(name, key, ep.done(function (user) {
    if (!user) {
      return res.render('notify/notify', {error: 'Incorrect activation link'});
    }
